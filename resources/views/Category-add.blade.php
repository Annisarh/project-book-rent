...
@section('content')
    {{-- bagian form start --}}
    <h1 class="font-bold my-5 text-3xl text-slate-800">#Category-add</h1>
     <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default mb-5">    
        <form class="w-3/4 space-y-4 p-8 flex items-center gap-6" action="" method="POST">
            @csrf
            <div class="flex-1">
                <label for="visitors" class="block mb-2.5 text-sm font-medium text-heading">Name Category</label>
                <input type="text" id="visitors" name="name" class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-2.5 py-2 shadow-xs placeholder:text-body @error('name') is-invalid @enderror" placeholder="inputkan name category" required />
                @error('name')
