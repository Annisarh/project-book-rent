...
<link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    {{-- bagian form start --}}
    <h1 class="font-bold my-5 text-3xl text-slate-800">#Book-add</h1>
     <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default mb-5">    
        <form class="w-3/4 space-y-4 p-8" action="" method="POST" enctype="multipart/form-data">
            @csrf
            <div>
                <label for="book_code" class="block mb-2.5 text-sm font-medium text-heading">Book Code</label>
                <input type="text" id="book_code" name="book_code" class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-2.5 py-2 shadow-xs placeholder:text-body @error('book_code') is-invalid @enderror" placeholder="inputkan book code" value="{{ old('book_code') }}" required />
                @error('book_code')
            ...
            <div>
                ...
                <input type="text" id="title" name="title" class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-2.5 py-2 shadow-xs placeholder:text-body @error('title') is-invalid @enderror" placeholder="inputkan book code" value="{{ old('title') }}" required />
                @error('title')
                    <div class="text-left text-red-400 w-full">{{ $message }}</div>
                @enderror
            </div>
            ...
            <button type="submit" class="text-white bg-brand box-border border border-transparent hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium rounded-base text-sm px-4 py-2.5 focus:outline-none mt-3">Simpan</button>
